...
frequency list =
  map (\x -> (length x, head x)) (group $ sort list)


rotateList :: [String] -> [String]
rotateList list =
 map newString [0..times]
  where
    newString n = map (!! n) list
    times = (length $ head list) - 1

sortFreq :: [(Int, Char)] -> [(Int, Char)]
sortFreq =
